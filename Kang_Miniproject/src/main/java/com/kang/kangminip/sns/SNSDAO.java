...
import com.kang.kangminip.SiteOption;
import com.kang.kangminip.member.Member;
...
public class SNSDAO {
	...
	public void getMsg(int page, HttpServletRequest req) {
		...
		for (SNSMsg snsMsg : snsMsgs) {
			snsMsg.setK_replys(ss.getMapper(SNSMapper.class).getReply(snsMsg));
		}
	...
	public void writeMsg(SNSMsg sm, HttpServletRequest req) {
		try {
			...
			sm.setK_id(m.getK_id());
			sm.setK_color(colors[new Random().nextInt(colors.length)]);
			String txt = sm.getK_txt();
			txt = txt.replace("\r\n", "<br>");
			sm.setK_txt(txt);
