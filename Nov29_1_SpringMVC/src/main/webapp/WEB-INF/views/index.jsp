...
<body>
	...
	Package명이 같으면 프로젝트가 달라도 주소가 같아질 수가 있어서
	에러 발생확률이 높으니 top-level package라는 이름이 중복되지 않도록 주의!!
	
	Spring 프로젝트 실행 -> 프로젝트 자체를 선택 후 ctrl + f11 -> Run on Server
	...
	Spring 구조
		...
		- src - main - webapp - WEB-INF
			...
			Spring 이라는 폴더  안에 root-context.xml
						: 워크스페이스 전체에 대한 설정
						appServlet - setvlet-context.xml
							: 현재 프로젝트에 대한 설정
			views폴더 : View를 작성하는 곳(.jsp)
			web.xml : 프로그램 실행시 함께 따라가는 설정들을 모아둔 파일
			pom.xml : 필요한 라이브러리들을 maven의 중앙저장소에서 가져와서 관리해주는 파일 (.jar파일을 maven형태로 저장)
